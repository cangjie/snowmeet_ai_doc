#!/usr/bin/env python3
"""按店铺 + 财年口径导出年度雪票（ski_pass）订单（单 sheet 宽表，财务/业务视角）。

仿 ../export_retail_order_fiscal_year（零售财年版）规则，换成雪票业务：
- 主表共用 [order]（o.type=N'雪票'）
- 行项目是 ski_pass（一张订单可含多张雪票），无成本/工序状态。
  雪票成交价 = SUM(ski_pass.deal_price)（实际成交价；含度假区标记）。
- 支付/退款/分账共用 order_payment/payment_refund/order_share/payment_share
- 行粒度 = 一行一订单（多张票在订单行聚合，不展开逐件明细）
- 营/非 照搬租赁雪季营业窗口（SEASON）

单 sheet「年度雪票」5 段拼接：固定前缀 + 动态支付区 + 动态退款区 + 固定中段(雪票特化) + 固定后缀。

用法：
    python export_ski_pass_orders_fy.py --shop 崇礼旗舰店
    python export_ski_pass_orders_fy.py --shop 崇礼旗舰店 --start 2025-05-01 --end 2026-04-30 --out x.xlsx
...
SELECT
    ...
    ISNULL(sp.skipass_cnt, 0)                               AS 雪票张数,
    ISNULL(sp.deal_sum, 0)                                  AS 成交价合计,
    ISNULL(sp.wanlong_cnt, 0)                               AS 万龙张数,
    ISNULL(sp.nanshan_cnt, 0)                               AS 南山张数,
    ISNULL((
        SELECT SUM(dd.amount) FROM (
            ...
            WHERE d.valid = 1 AND (
                  d.order_id = o.id
               OR (d.biz_type = N'雪票' AND d.biz_id IN (
                     SELECT sp2.id FROM ski_pass sp2 WHERE sp2.order_id = o.id AND sp2.valid = 1))
            )
        ) dd
    ), 0)                                                   AS 减免合计,
    ...
    ISNULL(sp.skipass_cnt, 0)                               AS _skipass_cnt
FROM [order] o
...
LEFT JOIN (
    SELECT sp2.order_id,
           COUNT(*) AS skipass_cnt,
           ISNULL(SUM(sp2.deal_price), 0) AS deal_sum,
           SUM(CASE WHEN sp2.resort = N'万龙' THEN 1 ELSE 0 END) AS wanlong_cnt,
           SUM(CASE WHEN sp2.resort = N'南山' THEN 1 ELSE 0 END) AS nanshan_cnt
    FROM ski_pass sp2 WHERE sp2.valid = 1
    GROUP BY sp2.order_id
...
MONEY_FIXED = {'支付合计', '退款合计', '订单结余', '成交价合计', '减免合计',
               '应分账金额', '实分账金额', '待分账金额'}
...
def derive_skipass_status(skipass_cnt, paid):
    """雪票无工序/履约状态，按订单级支付派生（🔶 简化口径）：
      - 无有效 ski_pass 行 → ''（临时单候选）
      - 有成功支付（支付合计>0）→ 已支付
      - 否则 → 未支付
    """
    if skipass_cnt == 0:
...
def parse_args():
    p = argparse.ArgumentParser(
        description='按店铺+财年导出年度雪票订单（单 sheet 宽表）',
        formatter_class=argparse.RawDescriptionHelpFormatter, epilog=__doc__)
    p.add_argument('--shop', required=True, help='店铺名（DB order.shop，如「崇礼旗舰店」「南山」「万龙」）')
    ...
    p.add_argument('--out', default=None, help='输出 xlsx 路径，默认 {prefix}_ski_pass_orders_fy_{start}_{end}.xlsx')
...
def default_out_name(shop, start, end):
    ...
    return f'{prefix}_ski_pass_orders_fy_{start}_{end}.xlsx'
...
def main():
    ...
    if missing:
        ...
        raise SystemExit(
            ...
            f'请在 export_ski_pass_orders_fy.py 的 SEASON dict 补对应「财年→(营业起,营业止)」后重跑。')
    ...
    for row in main_rows:
        ...
        balance = g('订单结余') or 0
        test_flag = g('测试')
        skipass_cnt = g('_skipass_cnt') or 0
        status = derive_skipass_status(skipass_cnt, paid)
        temp_order = '是' if (test_flag != '是' and balance > 0 and skipass_cnt == 0) else ''
        zheng_bi = '关闭' if (paid == 0) else '正常'

        seg1 = ['雪票', fy, ying, '', ops_day, g('日序号'), (bd.month if bd else None),
        ...
        seg4 = [skipass_cnt, round(float(g('成交价合计') or 0), 2), g('万龙张数') or 0,
                ...
                round(float(g('待分账金额') or 0), 2), '雪票', g('门店'),
