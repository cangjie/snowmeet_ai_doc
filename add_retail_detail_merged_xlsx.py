"""一次性产物：南山零售财年报表按「七色米订单号」合并展示商品明细。

匹配键：
    年度零售「七色米订单号」 == 销售明细单1「单据编号」（均为 XSDxxxx 格式）

输出形态（用户确认）：另存新文件 nanshan_retail_orders_fy_with_detail.xlsx，
原报表三 sheet 完全不动。新文件单 sheet「年度零售明细」：
    原 56 列（订单级） + 10 明细列（商品编号/商品名称/商品分类/规格/属性/
                                    数量/单价/折扣/折后单价/总额）
一单多商品 → 纵向展开 N 行，原 56 列用合并单元格跨 N 行；
无七色米号 / 未命中明细 → 单行，明细 10 列留空。

用法（确保产物未被 Excel 打开；源报表被打开不影响，只读加载）：
    py add_retail_detail_merged_xlsx.py
"""
import os
import sys
...
# 用户指定剔除：4 个非关闭无明细单中的测试单（测试='是'，¥0 未支付，店员陈晓庆）
EXCLUDE_CODES = {"NS_LS_251217_00004"}
CODE_HEADER = "订单号"
...
def load_source():
    """读年度零售 → (headers[56], data_rows[list[tuple]], key_idx0)。"""
...
def build(headers, data, key_idx0, dmap):
    ...
    末列每单仅 anchor 行有值（订单级，随 56 列一起合并）。
...
def main():
    ...
    print(f"销售明细单1: {n_docs} 单据 / {n_det_rows} 明细行")
    ...
    print(f"  剔除测试单 : {st['excluded']}  ({'/'.join(sorted(EXCLUDE_CODES))})")
    ...
    # === 2) 直接放入主报表（幂等：已存在则删重建；其余 3 sheet 不动）===
    ...
    # === 抽检 ===
    print("\n=== 抽检：合并跨度最大 / 一个 2 明细单 ===")
    by_span = sorted(merge_spans, key=lambda se: se[1] - se[0], reverse=True)
    for label, (s, e) in (("最大", by_span[0]),
                          ("2明细", next(se for se in merge_spans
                                          if se[1] - se[0] == 1))):
        code = ws.cell(row=s, column=headers.index("订单号") + 1).value
        mi7 = ws.cell(row=s, column=key_idx0 + 1).value
        print(f"  [{label}] 行{s}~{e} ({e - s + 1}件) 订单号={code} 七色米={mi7}")
        for rr in range(s, e + 1):
