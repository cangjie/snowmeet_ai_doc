-- 旧版 NVARCHAR 完整建表审阅稿；已被 2026-09-22_fnb_inventory_other_tables.sql（VARCHAR 版）取代，请勿再执行本文件。
-- 已处理订单来源字段时，只执行 2026-09-22_fnb_inventory_other_tables.sql。
-- SQL Server 2012+；一次执行涵盖 dbo.[order] 来源字段、旧食材表字符列、17 张新表和 2 个普通视图。
-- 请在已确认的目标业务数据库中执行整份脚本；所有变更处于同一个事务。
-- 如已执行旧版 2026-09-22_order_source_pair.sql，本脚本会把原 VARCHAR 来源字段升级为 NVARCHAR。
-- 一次性建表：任一目标对象已存在即报错，避免把已有不同结构误当作本版本。
-- 本脚本不创建旧表、不迁移旧批次数量、不预置食材保质期、不执行订单或库存业务。
-- 旧订单两个来源字段均为 NULL；来源字段的 CHECK 约束由使用者另行定义。
-- 本脚本新增／转换的字符列均为 NVARCHAR，以支持中文；业务枚举仍按 CHECK 中的代码值填写。
-- 字段字典：../docs/superpowers/specs/2026-09-21-fnb-inventory-schema-review.md
-- 新表 *_at 为 UTC；business_date/opened_date 与所有效期 DATE 按 Asia/Shanghai。
-- 数量=食材基本单位，金额=人民币元；ROWVERSION 不是日期，也不会自行防止超扣。
SET NOCOUNT ON;
SET XACT_ABORT ON;
SET ANSI_NULLS ON;
...
BEGIN TRY
    ...
    IF OBJECT_ID(N'dbo.fnb_material_alert_log', N'U') IS NULL
        THROW 51001, N'前置依赖 dbo.fnb_material_alert_log 不存在；请核查上次食材过期提醒建表情况。', 1;

    -- 复用的旧食材表：历史 DDL 中 6 个字符列为 VARCHAR，需纳入中文支持。
    -- 仅接受已知的原 VARCHAR 长度或同长度 NVARCHAR，避免覆盖其他线上改动。
    IF (SELECT COUNT(*) FROM sys.columns
        WHERE object_id = OBJECT_ID(N'dbo.fnb_material_batch')
          AND ((name = N'batch_no' AND is_nullable = 0 AND ((system_type_id = 167 AND max_length = 50) OR (system_type_id = 231 AND max_length = 100)))
            OR (name = N'image_ids' AND is_nullable = 1 AND ((system_type_id = 167 AND max_length = 500) OR (system_type_id = 231 AND max_length = 1000)))
            OR (name = N'dispose_userid' AND is_nullable = 1 AND ((system_type_id = 167 AND max_length = 64) OR (system_type_id = 231 AND max_length = 128)))
            OR (name = N'create_userid' AND is_nullable = 1 AND ((system_type_id = 167 AND max_length = 64) OR (system_type_id = 231 AND max_length = 128))))) <> 4
        THROW 51004, N'dbo.fnb_material_batch 字符列与历史定义不同；请先核查实际表结构。', 1;
    IF (SELECT COUNT(*) FROM sys.columns
        WHERE object_id = OBJECT_ID(N'dbo.fnb_material_alert_log')
          AND ((name = N'msgid' AND is_nullable = 1 AND ((system_type_id = 167 AND max_length = 100) OR (system_type_id = 231 AND max_length = 200)))
            OR (name = N'send_userid' AND is_nullable = 1 AND ((system_type_id = 167 AND max_length = 64) OR (system_type_id = 231 AND max_length = 128))))) <> 2
        THROW 51004, N'dbo.fnb_material_alert_log 字符列与历史定义不同；请先核查实际表结构。', 1;
    IF EXISTS (SELECT 1 FROM sys.index_columns AS ic
        ...
        WHERE ic.object_id = OBJECT_ID(N'dbo.fnb_material_batch') AND c.name = N'batch_no'
          AND ix.name <> N'IX_fnb_material_batch_batch_no'
          AND EXISTS (SELECT 1 FROM sys.columns AS bc
              WHERE bc.object_id = c.object_id AND bc.name = N'batch_no' AND bc.system_type_id = 167))
        THROW 51004, N'旧批次号存在额外索引；请先核查，再将 batch_no 转为 NVARCHAR。', 1;
    IF EXISTS (SELECT 1 FROM sys.indexes AS ix
        WHERE ix.object_id = OBJECT_ID(N'dbo.fnb_material_batch')
          AND ix.name = N'IX_fnb_material_batch_batch_no'
          AND (ix.is_unique = 1 OR ix.has_filter = 1
            OR (SELECT COUNT(*) FROM sys.index_columns AS ic
            ...
            OR NOT EXISTS (SELECT 1 FROM sys.index_columns AS ic
                ...
                WHERE ic.object_id = ix.object_id AND ic.index_id = ix.index_id
                  AND ic.key_ordinal = 1 AND c.name = N'batch_no')))
        THROW 51004, N'旧批次号索引定义与历史建表脚本不同；请先核查。', 1;
    DECLARE @source_column_count INT;
    DECLARE @source_state TINYINT = 0; -- 0=尚未添加，1=旧版 VARCHAR，2=已是 NVARCHAR
    SELECT @source_column_count = COUNT(*) FROM sys.columns
        WHERE object_id = OBJECT_ID(N'dbo.[order]') AND name IN (N'order_source', N'source_order_no');
    IF @source_column_count NOT IN (0, 2)
        THROW 51003, N'dbo.[order] 只有一个来源字段；请核查现有结构。', 1;
    IF @source_column_count = 2
    BEGIN
        IF (SELECT COUNT(*) FROM sys.columns
            WHERE object_id = OBJECT_ID(N'dbo.[order]') AND is_nullable = 1
              AND collation_name = N'Latin1_General_100_BIN2'
              AND ((name = N'order_source' AND system_type_id = 231 AND max_length = 64)
                OR (name = N'source_order_no' AND system_type_id = 231 AND max_length = 256))) = 2
            SET @source_state = 2;
        ELSE IF (SELECT COUNT(*) FROM sys.columns
            WHERE object_id = OBJECT_ID(N'dbo.[order]') AND is_nullable = 1
              AND collation_name = N'Latin1_General_100_BIN2'
              AND ((name = N'order_source' AND system_type_id = 167 AND max_length = 32)
                OR (name = N'source_order_no' AND system_type_id = 167 AND max_length = 128))) = 2
            SET @source_state = 1;
        ELSE
            THROW 51003, N'dbo.[order] 来源字段类型、长度、排序规则或可空性与本脚本不符。', 1;
    END;
    IF @source_state = 1 AND EXISTS (
        SELECT 1 FROM sys.index_columns AS ic
        JOIN sys.columns AS c ON c.object_id = ic.object_id AND c.column_id = ic.column_id
        WHERE ic.object_id = OBJECT_ID(N'dbo.[order]')
          AND c.name IN (N'order_source', N'source_order_no'))
        THROW 51003, N'旧版来源字段已有索引；请先核查，再升级为 NVARCHAR。', 1;
    ...
    -- 将上次食材过期提醒的非 Unicode 列纳入本系统；保留批次号索引。
    IF EXISTS (SELECT 1 FROM sys.columns
        WHERE object_id = OBJECT_ID(N'dbo.fnb_material_batch') AND name = N'batch_no' AND system_type_id = 167)
    BEGIN
        IF EXISTS (SELECT 1 FROM sys.indexes
            WHERE object_id = OBJECT_ID(N'dbo.fnb_material_batch') AND name = N'IX_fnb_material_batch_batch_no')
            DROP INDEX [IX_fnb_material_batch_batch_no] ON [dbo].[fnb_material_batch];
        ALTER TABLE [dbo].[fnb_material_batch] ALTER COLUMN [batch_no] NVARCHAR(50) NOT NULL;
        CREATE INDEX [IX_fnb_material_batch_batch_no] ON [dbo].[fnb_material_batch] ([batch_no]);
    END;
    IF EXISTS (SELECT 1 FROM sys.columns
        WHERE object_id = OBJECT_ID(N'dbo.fnb_material_batch') AND name = N'image_ids' AND system_type_id = 167)
        ALTER TABLE [dbo].[fnb_material_batch] ALTER COLUMN [image_ids] NVARCHAR(500) NULL;
    IF EXISTS (SELECT 1 FROM sys.columns
        WHERE object_id = OBJECT_ID(N'dbo.fnb_material_batch') AND name = N'dispose_userid' AND system_type_id = 167)
        ALTER TABLE [dbo].[fnb_material_batch] ALTER COLUMN [dispose_userid] NVARCHAR(64) NULL;
    IF EXISTS (SELECT 1 FROM sys.columns
        WHERE object_id = OBJECT_ID(N'dbo.fnb_material_batch') AND name = N'create_userid' AND system_type_id = 167)
        ALTER TABLE [dbo].[fnb_material_batch] ALTER COLUMN [create_userid] NVARCHAR(64) NULL;
    IF EXISTS (SELECT 1 FROM sys.columns
        WHERE object_id = OBJECT_ID(N'dbo.fnb_material_alert_log') AND name = N'msgid' AND system_type_id = 167)
        ALTER TABLE [dbo].[fnb_material_alert_log] ALTER COLUMN [msgid] NVARCHAR(100) NULL;
    IF EXISTS (SELECT 1 FROM sys.columns
        WHERE object_id = OBJECT_ID(N'dbo.fnb_material_alert_log') AND name = N'send_userid' AND system_type_id = 167)
        ALTER TABLE [dbo].[fnb_material_alert_log] ALTER COLUMN [send_userid] NVARCHAR(64) NULL;

    -- 支持首次添加，也支持将先前独立补列脚本的 VARCHAR 安全升级为 NVARCHAR。
    -- 不管理来源字段的 CHECK 约束；若旧约束阻碍 ALTER COLUMN，请先单独删除。
    IF @source_state = 0
    ...
    IF @source_state = 1
    BEGIN
        ALTER TABLE [dbo].[order] ALTER COLUMN [order_source] NVARCHAR(32) COLLATE Latin1_General_100_BIN2 NULL;
        ALTER TABLE [dbo].[order] ALTER COLUMN [source_order_no] NVARCHAR(128) COLLATE Latin1_General_100_BIN2 NULL;
    END;
    -- 01. fnb_unit：计量单位
    -- 定义录入单位与基本单位的换算；首版仅初始化克、千克、毫升、升、个。
    CREATE TABLE [dbo].[fnb_unit] (
        [code] NVARCHAR(16) NOT NULL, -- 单位编码：g、kg、ml、l、piece；主键
        [name] NVARCHAR(20) NOT NULL, -- 显示名称：克、千克、毫升、升、个
    ...
    CREATE TABLE [dbo].[fnb_material_category] (
        ...
        [default_storage] NVARCHAR(10) NULL, -- 二级分类建议储存：ambient=常温，chilled=冷藏，frozen=冷冻
        [default_unit_code] NVARCHAR(16) NULL, -- 二级分类默认录入单位，关联 fnb_unit.code
        [warn_days] INT NULL, -- 二级分类默认提前预警天数；复制到批次后可单独调整
        [default_open_storage] NVARCHAR(10) NULL, -- 默认开封后储存方式；实际值在入库时确认并保存在批次扩展表
    ...
    CREATE TABLE [dbo].[fnb_shelf_life_rule] (
        ...
        [storage_type] NVARCHAR(10) NOT NULL, -- ambient=常温，chilled=冷藏，frozen=冷冻
        [production_month] TINYINT NOT NULL, -- 生产月份，1～12；不分季节时配置 12 行相同规则
        [shelf_life_value] INT NOT NULL, -- 默认保质期数值，必须大于 0
        [shelf_life_unit] NVARCHAR(5) NOT NULL, -- day=天，month=月；写入旧批次字段时映射为中文天／月
        [remark] NVARCHAR(300) NULL, -- 规则依据及说明；本脚本不预设实际食材保质期
    ...
    CREATE TABLE [dbo].[fnb_material_item] (
        ...
        [code] NVARCHAR(32) NOT NULL, -- 系统食材编码，唯一且稳定
        [name] NVARCHAR(100) NOT NULL, -- 食材名称，例如高筋面粉、Pizza 面团
        [category_id] INT NOT NULL, -- 所属二级食材分类
        [item_type] NVARCHAR(16) NOT NULL, -- raw=原料，prepared=半成品
        [base_unit_code] NVARCHAR(16) NOT NULL, -- 唯一库存基本单位：g、ml、piece；有库存或流水后不可修改
        [default_input_unit_code] NVARCHAR(16) NOT NULL, -- 默认录入／显示单位，例如库存用 g、界面默认显示 kg
        [image_id] INT NULL, -- 档案参考图片，关联现有 mini_upload.id；不能替代入库现场照片
        [remark] NVARCHAR(500) NULL, -- 食材说明
    ...
    CREATE TABLE [dbo].[fnb_material_batch_stock] (
        ...
        [stock_form] NVARCHAR(10) NOT NULL, -- bulk=散装，sealed=未开封，opened=已开封，prepared=制作半成品
        [storage_type] NVARCHAR(10) NOT NULL, -- 本批次当前储存方式：ambient／chilled／frozen
        [storage_location] NVARCHAR(100) NULL, -- 储位文字，例如冷藏柜 A 层；首版不单建库位表
        ...
        [pack_unit_name] NVARCHAR(20) NULL, -- sealed 必填：瓶／袋／盒等包装单位，仅用于显示
        ...
        [open_storage_type] NVARCHAR(10) NULL, -- sealed／opened 必填：确认的开封后储存方式快照
        ...
        [expiry_source] NVARCHAR(12) NOT NULL, -- manual=手填，package=包装／OCR确认，category=分类规则，estimated=折算参考，opened=开封计算
        [expiry_note] NVARCHAR(500) NULL, -- 效期来源、折算依据、人工确认或冲突说明
    ...
    CREATE TABLE [dbo].[fnb_dish_spec] (
        ...
        [spec_code] NVARCHAR(32) NOT NULL, -- 菜品内部规格编码，例如 standard／large
        [name] NVARCHAR(50) NOT NULL, -- 规格名称，例如标准份、大份
    ...
    CREATE TABLE [dbo].[fnb_recipe] (
        ...
        [recipe_type] NVARCHAR(10) NOT NULL, -- dish=出餐配方，prep=半成品制作配方
        ...
        [status] NVARCHAR(10) NOT NULL DEFAULT ('draft'), -- draft=草稿，published=当前发布，retired=历史版本
        [remark] NVARCHAR(500) NULL, -- 配方说明
    ...
    CREATE TABLE [dbo].[fnb_channel_shop] (
        ...
        [platform] NVARCHAR(16) NOT NULL, -- meituan=美团，eleme=饿了么／淘宝闪购
        [external_shop_id] NVARCHAR(64) COLLATE Latin1_General_100_BIN2 NOT NULL, -- 平台门店 ID，按原始字符串保存，不转数字
        [external_shop_name] NVARCHAR(100) NULL, -- 平台门店名称快照
        [preferred_method] NVARCHAR(10) NULL, -- api／web／ocr；仅方案配置，不代表接口已开通或采集已实现
        [valid] BIT NOT NULL DEFAULT (1), -- 是否启用同步
        [last_success_at] DATETIME2(3) NULL, -- 最近成功同步时间，UTC；用于识别同步中断
        [last_error] NVARCHAR(500) NULL, -- 最近同步错误摘要，不保存密钥、Cookie 或验证码
        [created_at] DATETIME2(3) NOT NULL DEFAULT (SYSUTCDATETIME()), -- 创建时间，UTC
        [updated_at] DATETIME2(3) NULL, -- 修改时间，UTC
    ...
    CREATE TABLE [dbo].[fnb_channel_dish_map] (
        ...
        [external_sku_key] NVARCHAR(128) COLLATE Latin1_General_100_BIN2 NOT NULL, -- 平台商品／SKU 稳定标识；网页或 OCR 无标识时，人工确认后使用规范化业务键
        [option_key] NVARCHAR(128) COLLATE Latin1_General_100_BIN2 NOT NULL DEFAULT (''), -- 规格及选项的规范化键；无选项用空串，避免只按菜名误配
        [external_name] NVARCHAR(150) NOT NULL, -- 平台菜品名称，用于核对与展示
        [options_text] NVARCHAR(1000) NULL, -- 平台规格与加料描述，供人工核对
    ...
    CREATE TABLE [dbo].[fnb_order] (
        ...
        [source_type] NVARCHAR(10) NOT NULL, -- internal=本站餐饮订单，external=平台订单，manual=人工厨房单
        [sales_order_id] INT NULL, -- internal/external 必填：现有 dbo.[order].id；manual 可为空
        [channel_shop_id] INT NULL, -- external 必填：平台门店映射 ID
        [external_order_no] NVARCHAR(128) COLLATE Latin1_General_100_BIN2 NULL, -- external 必填：平台完整订单号；不能用每天重复的小票流水号替代
        [display_no] NVARCHAR(64) NOT NULL, -- 展示单号／取餐号；不作为外卖去重依据
        [business_date] DATE NOT NULL, -- 上海时区营业日期，由服务端确定
        [ordered_at] DATETIME2(3) NOT NULL, -- 下单时间，UTC；导入时从平台时区转换
        [table_no] NVARCHAR(50) NULL, -- 堂食桌号或取餐位置
        [order_status] NVARCHAR(12) NOT NULL, -- pending=待接，accepted=已接，completed=平台完成，cancelled=取消；与库存出餐过账状态分开
        [platform_status] NVARCHAR(50) NULL, -- 平台原始状态文本／编码，便于适配及追溯
        [refund_status] NVARCHAR(10) NOT NULL DEFAULT ('none'), -- none=无退款，partial=部分退款，full=全额退款；不直接代表应恢复库存
        [total_amount] DECIMAL(19,4) NULL, -- 来源订单金额快照，元；仅展示，不用于本系统收款结算
        [refund_amount] DECIMAL(19,4) NULL, -- 来源退款金额快照，元；部分来源缺失时为空
        [review_status] NVARCHAR(10) NOT NULL DEFAULT ('pending'), -- pending=待核对，verified=已核对；完整可信 API 数据可由系统核对
        [remark] NVARCHAR(1000) NULL, -- 订单制作备注，例如不要葱
    ...
    CREATE TABLE [dbo].[fnb_order_line] (
        ...
        [line_key] NVARCHAR(128) COLLATE Latin1_General_100_BIN2 NOT NULL, -- 单内稳定明细键；有平台明细 ID 时使用该 ID，否则由适配器规范化生成
        [parent_line_id] BIGINT NULL, -- 套餐／加料的父行 ID，必须属于同一订单
        [legacy_fd_order_id] INT NULL, -- 原有餐饮明细 fd_order.id，用于内部订单关联
        [external_sku_key] NVARCHAR(128) COLLATE Latin1_General_100_BIN2 NULL, -- 平台菜品／SKU 标识
        [option_key] NVARCHAR(128) COLLATE Latin1_General_100_BIN2 NOT NULL DEFAULT (''), -- 规范化规格与选项键
        [item_name] NVARCHAR(150) NOT NULL, -- 菜品名称快照
        [spec_name] NVARCHAR(100) NULL, -- 规格名称快照
        [options_text] NVARCHAR(1000) NULL, -- 加料、套餐组成与选项说明
        ...
        [remark] NVARCHAR(500) NULL, -- 单行制作备注
    ...
    CREATE TABLE [dbo].[fnb_order_import] (
        ...
        [source_method] NVARCHAR(10) NOT NULL, -- api、web、ocr、internal
        [dedupe_key] NVARCHAR(128) COLLATE Latin1_General_100_BIN2 NOT NULL, -- 本次事件／采集内容的规范化去重键；同一原始通知重试保持一致
        [order_id] BIGINT NULL, -- 成功匹配／创建的厨房订单；待核对或解析失败时可为空
        [upload_id] INT NULL, -- OCR 截图关联 mini_upload.id；多张截图分别记录，再匹配同一订单
        [raw_content] NVARCHAR(MAX) NULL, -- 经必要裁剪的原始订单数据／OCR 文本；不保存登录凭证和无关顾客信息
        [parsed_content] NVARCHAR(MAX) NULL, -- 待核对的结构化解析结果，JSON 文本；SQL Server 2012 下由应用校验格式
        [process_status] NVARCHAR(12) NOT NULL DEFAULT ('pending'), -- pending=待处理，accepted=已导入，review=待人工核对，failed=失败，ignored=重复或旧消息
        [error_message] NVARCHAR(1000) NULL, -- 失败原因、字段缺失或冲突说明
    ...
    CREATE TABLE [dbo].[fnb_stock_document] (
        ...
        [document_no] NVARCHAR(40) NOT NULL, -- 系统库存单号，同店唯一；与包装上可重复的 batch_no 不同
        [document_type] NVARCHAR(16) NOT NULL, -- receipt=入库，open=开封，prep=制作，serve=出餐，waste=报损／销毁，stocktake=盘点，opening_balance=旧批次期初接管
        [status] NVARCHAR(10) NOT NULL DEFAULT ('draft'), -- draft=未过账，posted=已过账，void=未过账作废；已过账单不可直接改为作废
        [request_id] UNIQUEIDENTIFIER NOT NULL, -- 调用方同一次操作固定的请求 ID；重试不得生成新值
        [source_client] NVARCHAR(10) NOT NULL, -- mini=小程序，wecom=企业微信，system=系统处理
        [business_date] DATE NOT NULL, -- 上海时区营业日期，由服务端确定
        ...
        [reason_code] NVARCHAR(16) NULL, -- waste 必填：expiry=过期销毁，near_expiry=临期报损，damage=损坏，other=其他
        [reference_no] NVARCHAR(100) NULL, -- 入库送货单号或其他人工外部凭据
        [remark] NVARCHAR(1000) NULL, -- 单据说明；零成本入库、盘盈成本和效期依据在此说明
    ...
    CREATE TABLE [dbo].[fnb_stock_document_line] (
        ...
        [remark] NVARCHAR(500) NULL, -- 用料、欠料或价格说明
    ...
    CREATE TABLE [dbo].[fnb_stocktake_line] (
        ...
        [remark] NVARCHAR(500) NULL, -- 盘点差异说明；新建盘盈批次的效期与成本须另行确认
